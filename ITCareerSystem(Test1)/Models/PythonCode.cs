...
namespace ITCareerSystem_Test1_.Models
{
    ...
    {
        public static object PythonEngine { get; private set; }
        ...
        public string GetPythonValue(string csvPath)
        {
            try
            {
                ...
                scope.SetVariable("csv_path", csvPath);
...
try:
    ...
    rounded_z_score = round(new_year_z_score[0], 4)

    # Print the rounded predicted Z score
    print(rounded_z_score)

except Exception as e:
    print(f""Error: {e}. Please fix the dataset and try again."")", scope);
                    dynamic result = scope.GetVariable("rounded_z_score");
                         return result;
